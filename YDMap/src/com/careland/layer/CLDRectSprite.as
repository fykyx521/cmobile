...
//			g.clear();
//			var lp:Point=this.globalToLocal(firstPoint);
//			var np:Point=new Point(e.localX,e.localY).subtract(lp);
//			g.beginFill(StyleDraw.drawStyle,.3);
//			g.drawRect(lp.x,lp.y,Math.abs(np.x),Math.abs(np.y));
//			g.endFill();
...
//			this.removeEventListener(MouseEvent.MOUSE_MOVE,moveHandler);
			...
			cle.sourcepoint=parent.globalToLocal(firstPoint);
			cle.toPoint=new Point(e.stageX,e.stageY).subtract(firstPoint);
			cle.isShow=true;
