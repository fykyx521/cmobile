...
//				marker.x=np.x;
//				marker.y=np.y;
				marker.change(centerPoint,vscale,sw,sh,globel,mapConfig);
			}
			
			for each(var line:CLDPolyLine in polylines){
				line.change(centerPoint,vscale,sw,sh,globel,mapConfig);
			}
			for each(var gon:CLDPolygon in polygons){
				gon.change(centerPoint,vscale,sw,sh,globel,mapConfig);
			}
			
			for each(var circle:CLDCircle in circles){
				circle.change(centerPoint,vscale,sw,sh,globel,mapConfig);
			}
			//changeLine(centerPoint);
		}
		//根据屏幕上相对坐标获取careland坐标
		protected function getCLDPoint(screenPoint:Point,center:Point,vw:Number,vh:Number,mapConfig:MapConfig):Point
		{
		...
		public function getScreenPoint(cldPoint:Point,center:Point,vscale:Number,sw:Number,sh:Number,mapConfig:MapConfig):Point
		{
			var stagePoint:Point=CLDConfig.instance().toScreenPoint(cldPoint.x,cldPoint.y,sw,sh,vscale,center.x,center.y,mapConfig.tilemapindex,mapConfig.wscale,mapConfig.hscale);
