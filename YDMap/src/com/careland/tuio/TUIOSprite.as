...
package com.careland.tuio{
	...
	public dynamic class TUIOSprite extends MovieClip
	{
...
//				//设置相关变量
			...
			if (blobs.length >=3)
			{
				...
				if(!tuioobj3)
				{
					removeBlob(blobs[blobs.length-1].id);
					return;
				}
		...
		private function removeBlob(id:Number):void
		{
			for(var i:Number=0; i<blobs.length; i++)
			{
				if(blobs[i].id == id) 
				{
					...
					//modify TUIOObject，添加了一个oldX，跟oldY的属性。表示该点down时的坐标。
					...
					{						
						...
						if (distance < clickRadius*clickRadius)
						{
							...
							//this.dispatchEvent(new TouchEvent(TouchEvent.CLICK, true, false, tuioobj.x, tuioobj.y, localPoint.x, localPoint.y, 0, 0, this, false, false, false, true, 0, tuioobj.TUIOClass, tuioobj.ID, tuioobj.sID, tuioobj.angle));
							//this.dispatchEvent(new TUIOEvent(TUIOEvent.TUIO_CLICK, true, false, tuioobj.x, tuioobj.y, localPoint.x, localPoint.y, 0, 0, tuioobj.oldX, tuioobj.y, this, false, false, false, true, 0, tuioobj.TUIO_TYPE, tuioobj.ID, tuioobj.sID, tuioobj.angle));
					...
					{
						state = "none";
						lastPointUpTime=getTimer();
						if (lastPointUpTime-preLastPointUpTime>200)
							dAng=0;						
						if (this.noFloat)	return;	
					...
					if(blobs.length == 1)  
					{
						...
						blob1 = blobs[0];	
						//对比最后两个点抬起的时间，如果时间超过200毫秒，那么dAng清零，否则dAng保持
						//如果在这里加上清零，那么角度旋转就没有了惯性
						preLastPointUpTime=getTimer();
					...
					if(blobs.length == 2) {	
						state = "rotatescale";	
						if (blob1.id!=id && blob2.id!=id)	return;
...
//------------多点触控消息处理--------------------------------------------------------------------------------
		...
		{		
			...
			if(bringToFront)
				this.parent.setChildIndex(this, this.parent.numChildren-bringToFrontNum);
		...
		public function clickEvent(e:TUIOEvent):void
		{
			...
			else
			{
				...
				if (Math.abs(lastClickTime-getTimer())<this.doubleclickDuration && distance < clickRadius*clickRadius)
				{
					...
					{
						var localPoint:Point = this.parent.globalToLocal(new Point(x, y));							
						//this.dispatchEvent(new TUIOEvent(TUIOEvent.DOUBLE_CLICK, true, false, tuioobj.x, tuioobj.y, localPoint.x, localPoint.y, 0, 0, this, false, false, false, true, 0, tuioobj.TUIOClass, tuioobj.ID, tuioobj.sID, tuioobj.angle));
					//	this.dispatchEvent(new TUIOEvent(TUIOEvent.DOUBLE_CLICK,true,false,tuioobj.x,tuioobj.y,localPoint.x,localPoint.y,0,0,tuioobj.oldX,tuioobj.oldY,this,false,false,false,true,0, tuioobj.TUIO_TYPE, tuioobj.ID, tuioobj.sID, tuioobj.angle))
					}
				}
			}
...
//end----------多点触控消息处理--------------------------------------------------------------------------------
		...
		private function update(e:Event):void
		{
			...
			{				
				...
				{	
					ObjectPosition=new Point(x,y);
					lineBO=ObjectPosition.subtract(LastCenterPoint);
					lenBO=Point.distance(ObjectPosition,LastCenterPoint);
					
					 angBO=this.getAngleTrig(lineBO.x,lineBO.y);
					 O1=Point.polar(lenBO*MinScale/scaleX,angBO/180*Math.PI)
				...
				if(scaleX - MaxScale>0.010)
				{
					...
					lenBO=Point.distance(ObjectPosition,LastCenterPoint);
					angBO=this.getAngleTrig(lineBO.x,lineBO.y);
					O1=Point.polar(lenBO*MaxScale/scaleX,angBO/180*Math.PI)
					O1=O1.add(LastCenterPoint);
					ScaleLimitTweenerflag=false;
					//trace("x:",x," to-x",O1.x);
					//trace("y:",y," to-y",O1.y);
