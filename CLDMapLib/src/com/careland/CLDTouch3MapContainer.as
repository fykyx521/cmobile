...
package com.careland
{
	...
	public dynamic class CLDTouch3MapContainer extends CLDMapContainer
	{
...
//----------设置对象的各个属性---------------------------------------
		...
		private function getScreenPoint(center:Point, vscale:Number, sw:Number, sh:Number):Point
		{
			...
			var stagePoint:Point=CLDConfig.instance().toScreenPoint(center.x, center.y,mapConfig);
...
//					bit.filters=[blur];
		...
		public function toScreenPoint(x,y,cf):Point
		{
			return this.config.toScreenPoint(x,y,cf);
		} 
		public function toMapPoint(x,y,cf):Point
		{
			return this.config.toMapPoint(x,y,cf);
		}
		/*
		----------------从队列去掉一个触点--------------------------------------------
		*/
...
//						preY=curPosition.y = this.content.y;
			...
			if (blobs.length == 0)
			{
				...
				if(moveDistance>30){
					...
					var screenPoint:Point=this.toScreenPoint(this.newCenter.x,this.newCenter.y,this.mapconfig);
					...
					var np:Point=this.toMapPoint(np0.x,np0.y,mapconfig);
			...
			if (blobs.length == 0 && isTwo)
			{
				...
				if(touchSprite.scaleX>1.5){
					this.zoom++;isChange=true;
					
					isBig=true;
					
				}else if(touchSprite.scaleX<1){
					this.zoom--; isChange=true;
