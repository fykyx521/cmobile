package com.careland.component.layout
{
	...
	import com.careland.component.util.Padding;
	...
	public class CLDLayoutBase extends CLDBaseComponent
	{
		//触控需要的属性
		...
		private var cldbtn:CLDTouchButton;
		...
		public var isConstom:Boolean=false;//是否 是自定义窗口
		
		public function CLDLayoutBase(parent:DisplayObjectContainer=null, xpos:Number=0, ypos:Number=0, autoLoad:Boolean=false, timeInteval:int=0,isSwitchWin:Boolean=true)
		{
			super(parent, xpos, ypos, autoLoad, timeInteval);
			
			this.isSwitchWin=isSwitchWin;
			if(isSwitchWin){
		 ...
		 protected function isImgMap():void
		{
		 ...
		 protected function isMap():void
		{
			
		}
		
		private function pauseXML(data:*):void
		{
			...
			for(var i:int=0;i<xml.data.length();i++){
				...
				if(this.isConstom){
					...
					switch(type){
						case "4":this.isMap();isMapwin=true;break;
...
//				win.style.width = this.width * _w;
//        		win.style.height = this.height * _h;
//      			  win.style.left = _w / this.x;
//       			 win.style.top = _h / this.y;

				var cldw:CLDWindow=new CLDWindow();
				
				cldw.setSize(500,500);
				
				cldw.index=xml.data[i].attribute("窗口ID");
				...
				cldw.setSize(winWidth,winHeight);
		...
		private function winUp(e:CLDEvent):void
		{
...
//					Tweener.addTween(win,{width:0,height:0,alpha:.1,time:.3,delay:.5,onComplete:function(){
...
//			}
		...
		private function removeBlob(id:Number):void
		{
			...
			//不是自定义视图
			if(blobs.length==0&&!this.isConstom){
				while(this.layoutSprite.numChildren>0)this.layoutSprite.removeChildAt(0);
...
//			}
		...
		{
			super.dispose();
			this.stage.removeEventListener(TUIOEvent.TUIO_UP,upHandler);
			this.removeEventListener(Event.ENTER_FRAME,update);
			this.removeEventListener(TUIOEvent.TUIO_UP,upHandler);
		}
