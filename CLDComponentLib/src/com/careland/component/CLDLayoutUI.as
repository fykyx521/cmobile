package com.careland.component
{
	...
	public class CLDLayoutUI extends CLDBaseComponent
	{
		...
		//本身属性
		protected var _spacing:Number = -10; //原先是5
		...
		private var oldParam:Object; //窗体旧的 坐标宽高 
...
//			this.addEventListener(TUIOEvent.TUIO_DOWN,layoutDown,true);//加个true 会和window本身的事件冲突
		...
		private function addBlob(id:Number, origX:Number, origY:Number):void
		{
			...
			if(blobs.length==1&&!this.isMax){
				...
				if(cld){
					...
					this.addRect(cld,new Point(origX-p.x,origY-p.y),id);
		...
		private function addRect(cld:CLDWindow,disP:Point,id:int):void
		{
...
//					case "5":this.isImgMap();isMapwin=false;break;
				...
				cldw.addEventListener(CLDEvent.WIN_ZOOM_START,winZoom);
				cldw.addEventListener(CLDEvent.WIN_ZOOM_ING,winZoom);
				cldw.addEventListener(CLDEvent.WIN_ZOOM_END,winZoom);
				
				if(this.isConstom){
					cldw.addEventListener(CLDEvent.WINUP,winUp);
					cldw.addEventListener(CLDEvent.WINIPADUP,winIpadUp);
					cldw.addEventListener(CLDEvent.WINCLOSEWIN,closeWin);
		...
		private function initOldParam(win:CLDWindow):void
		{
			this.oldParam=new Object;
			this.oldParam.x=win.x;
			this.oldParam.y=win.y;
			this.oldParam.width=win.width;
			this.oldParam.height=win.height;
		}
		/**
		 *  
		 * */
		protected  function winZoom(e:CLDEvent):void
		{
			if(this.isConstom)
			{
				return ;//不是自定义布局的话  return;
			}
			var win:CLDWindow=e.target as CLDWindow;
			
			var gl:Point=new Point(win.x,win.y);
			 switch(e.type)
			 {
				 case CLDEvent.WIN_ZOOM_START: 
					 scaleWin=win;
					 if(!this.isMax)
					 {
						 initOldParam(scaleWin);
					 }
					 break;
				 case CLDEvent.WIN_ZOOM_ING:  
					 break;
				 case CLDEvent.WIN_ZOOM_END: 
					 if(!isMax&&this.scaleWin.width>this.oldParam.width)
					 {
						  this.toMax(win);
						  var mes:Message=Message.buildMsg(CMD.WINMAX);
						  mes.data={winIndex:scaleWin.winIndex};
						  this.sendCommand(mes);
					 }else if(isMax&&this.scaleWin)
					 {
						 if(this.scaleWin.width<this.width)
						 {
							 this.toBack(scaleWin);
							 var mes:Message=Message.buildMsg(CMD.WINBACK);
							 mes.data={winIndex:scaleWin.winIndex};
							 this.sendCommand(mes);
						 }else
						 {
							 Tweener.addTween(win,{x:0,y:0,width:this.width,height:this.height});
						 }
...
//					 this.removeChild(rect);
		...
		{
			super.register();
			this.registerCommand(CMD.WINMAX,CMD.WINBACK);
		}
		override public function unregister():void
		{
			super.unregister();
			this.unregisterCommand(CMD.WINMAX,CMD.WINBACK);
		}
		override protected function handlerRemote(e:Message):void
		{
			
			if(e.type==CMD.WINMAX)
			{
				 var win:CLDWindow=this.getWinByIndexId(e.data.winIndex);
				 if(win)
				 	this.initOldParam(win);
				 this.toMax(win);
			}else if(e.type==CMD.WINBACK)
			{
				var win:CLDWindow=this.getWinByIndexId(e.data.winIndex);
				this.toBack(win);
			}
		}
		 protected function getWinByIndexId(winIndex:int):CLDWindow
		{
			 for(var x:int=0;x<this.numChildren;x++){
				 var zw0:CLDWindow=this.getChildAt(x) as CLDWindow;
				 if(zw0&&zw0.winIndex==winIndex)
				 {
					 return zw0;
				 }
			 }
		 ...
		 private function toMax(win:CLDWindow)
		 {
			if(win)
			{
				Tweener.addTween(win,{x:0,y:0,width:this.width,height:this.height});
		 ...
		 private function toBack(win:CLDWindow):void
		 {
			 if(win)
			 {
				 var oldP:Object=oldParam;
				 Tweener.addTween(win,{x:oldP.x,y:oldP.y,width:oldP.width,height:oldP.height,time:0.4,transition:"easeinoutback"});
				 isMax=false;
				 for(var x:int=0;x<this.numChildren;x++){
					 var zw0:CLDWindow=this.getChildAt(x) as CLDWindow;
					 zw0.visible=true;
				 }
				 this.layout();//重绘校正  可能出现的 问题 
			 }
		 }
		
		/**
		 *  ipad 拖动结束事件  判断是否到了 垃圾箱附近
		 * */
		protected function winIpadUp(e:CLDEvent):void
		{
			var win:CLDWindow=e.target as CLDWindow;
			
			if(win){
				if(win.y+win.height>this.height&&this.x<145)
				{
					var cld:CLDEvent=new CLDEvent(CLDEvent.WINCLOSEWIN);
					win.dispatchEvent(cld);
					//发送窗体关闭消息 
					var mes:Message=Message.buildMsg(CMD.WINCLOSE)
					mes.data={winIndex:win.winIndex};
					this.sendCommand(mes);
				}
			}
		}
			...
			if(win){
				if(e.stageX<145&&e.stageY>stageHeight-145){
					Tweener.addTween(win,{alpha:.5,time:.5});
					 ...
					 win.removeEventListener(CLDEvent.WINIPADUP,winIpadUp);
					 var cld:CLDEvent=new CLDEvent(CLDEvent.WINCLOSEWIN);
					 win.dispatchEvent(cld);
		 ...
		 * 关闭窗口
		 * */
		public  function closeWin(e:CLDEvent):void
		{
			var win:CLDWindow=e.target as CLDWindow;
			if(win){
				win.removeEventListener(CLDEvent.WINCLOSEWIN,closeWin);
