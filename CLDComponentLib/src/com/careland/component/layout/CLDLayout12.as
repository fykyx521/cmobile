...
//			map.addEventListener(MapEvent.MapLocationChange,mapLocationChange);
			if(param){
				this.setMapType(dt);
			}else{
		...
		override protected function layout():void
		{
			...
			super.layout();
			map.setSize(this.width,this.height);
			if(this.backWin&&this.backWin.isBack)
			{
				this.backWin.setSize(this.width,this.height);
			}
...
//			backWin.constom=false; 
			...
			if(this.numChildren>1){
				this.setChildIndex(backWin,this.numChildren-1);
...
//				map.removeEventListener(MapEvent.MapLocationChange,mapLocationChange);
				map.dispose();
			}
			if(this.mapOver){
				mapOver.dispose();
