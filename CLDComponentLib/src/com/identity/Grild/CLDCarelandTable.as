...
//	    private var loading:GIFPlayer;
		...
		private var lrPan:PanGesture;//左右拖动 
		private var udPan:PanGesture;//上下拖动
...
//			this.removeEventListener(TUIOEvent.TUIO_MOVE,mouseMove);
			...
			if(lrPan)
			{
				lrPan.removeAllEventListeners();
				lrPan.dispose();
				lrPan=null;
			}
			if(udPan)
			{
				udPan.removeAllEventListeners();
				udPan.dispose();
				udPan=null;
			}
		}
...
// 		    loading.x=this.width/2;
		...
		private function build():void
		{
			...
			this.addChild(right);
			up=new Bitmap();
			up.bitmapData=getBitmap("upPoint").bitmapData;
			up.visible=false;
			...
			down=new Bitmap();
			down.bitmapData=getBitmap("downPoint").bitmapData;
			down.visible=false;
			down.y=this.maxY;
			down.x=(this.width - down.width) / 2;
			this.addChild(down);
...
//			this.addEventListener(MouseEvent.MOUSE_OUT,mouseOut);
		//	this.addEventListener(TUIOEvent.TUIO_OUT, TUTIMouseOut);
			...
			if(!config.webConfig)//ipad下的拖动
			{
				lrPan=new PanGesture(body);
				lrPan.maxNumTouchesRequired=1;
				lrPan.direction=PanGestureDirection.HORIZONTAL;
				lrPan.addEventListener(org.gestouch.events.GestureEvent.GESTURE_BEGAN,leftRightPan);
				lrPan.addEventListener(org.gestouch.events.GestureEvent.GESTURE_CHANGED,leftRightPan);
				lrPan.addEventListener(org.gestouch.events.GestureEvent.GESTURE_ENDED,leftRightPan);
				
				
				udPan=new PanGesture(body);
				udPan.maxNumTouchesRequired=1;
				udPan.direction=PanGestureDirection.VERTICAL;
				udPan.addEventListener(org.gestouch.events.GestureEvent.GESTURE_BEGAN,upDownPan);
				udPan.addEventListener(org.gestouch.events.GestureEvent.GESTURE_CHANGED,upDownPan);
				udPan.addEventListener(org.gestouch.events.GestureEvent.GESTURE_ENDED,upDownPan);
			}
		...
		private function leftRightPan(e:org.gestouch.events.GestureEvent):void
		{
			const gesture:PanGesture=e.target as PanGesture;
			switch(e.type)
			{
				case org.gestouch.events.GestureEvent.GESTURE_BEGAN: 
				case org.gestouch.events.GestureEvent.GESTURE_CHANGED: 
					this.body.x+=gesture.offsetX;
					if(body.x>100){
					...
					break;
				case org.gestouch.events.GestureEvent.GESTURE_ENDED:
					if(body.x<-100){
						direction="left";
					}
					if(body.x>100){
					...
					this.loadByDirection(direction);
					break;
					
			}
		}
		
		private function upDownPan(e:org.gestouch.events.GestureEvent):void
		{
			const gesture:PanGesture=e.target as PanGesture;
			switch(e.type)
			{
		...
		private function loadByDirection(direction:String):void
		{
			this.sendGridCommand(direction);
			loadDataByDirection(direction);
		}
		private function loadDataByDirection(direction:String):void
		{
			this.hiddenUI();
			if(direction!=""){
			...
			}else if(body.x!=0||body.y>0||this.body.y+this.maxY<this.height)
			{
				Tweener.addTween(body, {x: 0,y:0,time: 0.3, onComplete: onComplete});
			}
		}
		
		private function sendGridCommand(direction:String):void
		{
			 var mes:Message=Message.buildMsg(CMD.GRID_PAGE);
			 mes.data={direction:direction};
...
//						body.next(); //下一页
		...
		{
			 super.register();
			 this.registerCommand(CMD.GRID_PAGE);
		}
		override public function unregister():void
		{
			super.unregister();
			this.unregisterCommand(CMD.GRID_PAGE);
		}
		override protected function handlerRemote(e:Message):void
		{
			 if(CMD.GRID_PAGE==e.type)
			 {
				 this.loadDataByDirection(e.data.direction);
			 }
		}
