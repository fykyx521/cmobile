	...
	public class MouseGesture extends EventDispatcher {
		...
		*/
		...
		public function pause():void
		{
			this.timer.stop();
			gestures=[];
			timer.removeEventListener(TimerEvent.TIMER,captureHandler);
			mouseZone.removeEventListener(MouseEvent.MOUSE_DOWN,startCapture);
			mouseZone.removeEventListener(MouseEvent.MOUSE_UP,stopCapture);
		}
		...
		*/
		protected function matchGesture():void{
			...
			var vrect:Rectangle=null;
			try{
				vrect=new Rectangle(	rect.minx,
								rect.miny,
								rect.maxx-rect.minx,
								rect.maxy-rect.miny);
			}catch(e:Error){
			...
			var isDis:Boolean=true;
//			if(this.points.length>0){
//				isDis=false;
//				var firstPoint:Point=this.points[0];
//				var dis:Point=this.lastPoint.subtract(firstPoint);
//				if(Math.abs(dis.x)<100&&Math.abs(dis.y)<100){
//					isDis=true;
//				}
//			}
			
			if (bestGesture!=null&&isDis){
