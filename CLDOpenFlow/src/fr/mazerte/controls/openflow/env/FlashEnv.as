...
package fr.mazerte.controls.openflow.env
{
	...
	import fr.mazerte.controls.openflow.utils.ArrayUtil;
	...
	public class FlashEnv extends Sprite implements IEnv
	{
		...
		public function addItem(item:IItemRenderer, seek:Number, index:int, withAnim:Boolean=true):void
		{
			...
			//itemContainer.doubleClickEnabled=false;
			...
			//itemContainer.addEventListener(MouseEvent.DOUBLE_CLICK, _itemMouseDoubleClickHandler);
			
			
			itemContainer.addEventListener(TUIOEvent.TUIO_DOWN,_itemTouchDown);
			itemContainer.addEventListener(TUIOEvent.TUIO_MOVE,_itemTouchMove);
			itemContainer.addEventListener(TUIOEvent.TUIO_UP,_itemTouchUp);
			
			
			itemContainer.addEventListener(CLDTwoFingerEvent.CLD_TWO_FINGER_TAB,_itemDoubleTouch);
		...
		public function removeItem(item:IItemRenderer, withAnim:Boolean=true):void
		{
			for(var i:uint = 0; i < _items.length; i++)
			{
				...
				{			
					...
					Sprite(_items[i]).removeEventListener(MouseEvent.DOUBLE_CLICK, _itemMouseDoubleClickHandler);
					if(withAnim)
					{
		...
		private function _itemTouchDown(e:TUIOEvent):void
		{
			
		}
		private function _itemTouchMove(e:TUIOEvent):void
		{

		}
		private function _itemTouchUp(e:TUIOEvent):void
		{
			
		}
		private function _itemMouseDoubleClickHandler(e:MouseEvent):void
		{
			
			this.dispatchEvent(e);	
			
		}
		private function _itemDoubleTouch(e:CLDTwoFingerEvent):void
		{
