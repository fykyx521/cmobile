package uk.co.teethgrinder.charts.series {
	...
	public class Element extends Sprite implements has_tooltip {
		...
		public function resize(sc:ScreenCoordsBase,w:Number,h:Number):void {
		...
		private function mouseUp(event:Event):void {
			...
			if ( this.link.substring(0, 6) == 'trace:' ) {
				...
				tr.ace("call "+this.link+",index="+this.index);
		...
		public function get_tip_pos():Object {
			//var p:flash.geom.Point = this.localToGlobal( new flash.geom.Point(this.x, this.y) );
			//return {x:p.x,y:p.y};
