package uk.co.teethgrinder.elements.axis {
	import flash.display.Sprite;
	import flash.geom.Matrix;
	import uk.co.teethgrinder.string.StringUtils;
	import uk.co.teethgrinder.charts.series.bars.Bar3DElement;
	import uk.co.teethgrinder.Range;
	import uk.co.teethgrinder.Range;
	...
	public class XAxis extends Sprite {
		...
		function XAxis( json:Object, min:Number, max:Number )
		{
			...
			this.colour = StringUtils.get_colour( this.style.colour );
			this.grid_colour = StringUtils.get_colour( this.style['grid-colour'] );
			...
			if( json )
			{
				if( json.x_label_style != undefined ) {
					...
					if( json.x_label_style.alt_axis_colour != undefined )
						this.alt_axis_colour = StringUtils.get_colour(json.x_label_style.alt_axis_colour);
				}
			}
				...
				for each( var lbl:Object in this.style.labels.labels )
				{
					if (!(lbl is String)) {
						if (lbl.x != null) 
						{
							...
							if (lbl["grid-colour"])
							{
								tmpObj["grid-colour"] = StringUtils.get_colour(lbl["grid-colour"]);
							}
							else
							{
		...
		public function three_d_axis( sc:ScreenCoords ):void
		{
			...
			var lighter:Number = Bar3DElement.Lighten( this.colour );
