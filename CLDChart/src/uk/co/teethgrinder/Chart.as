package uk.co.teethgrinder  {
	...
	import uk.co.teethgrinder.charts.Factory;
	import uk.co.teethgrinder.charts.ObjectCollection;
	import uk.co.teethgrinder.charts.series.Element;
	import uk.co.teethgrinder.charts.series.has_tooltip;
	import uk.co.teethgrinder.elements.Background;
	import uk.co.teethgrinder.elements.axis.RadarAxis;
	import uk.co.teethgrinder.elements.axis.XAxis;
	import uk.co.teethgrinder.elements.axis.XAxisLabels;
	import uk.co.teethgrinder.elements.axis.YAxisBase;
	import uk.co.teethgrinder.elements.axis.YAxisLeft;
	import uk.co.teethgrinder.elements.axis.YAxisRight;
	import uk.co.teethgrinder.elements.labels.Keys;
	import uk.co.teethgrinder.elements.labels.Title;
	import uk.co.teethgrinder.elements.labels.XLegend;
	import uk.co.teethgrinder.elements.labels.YLegendBase;
	import uk.co.teethgrinder.elements.labels.YLegendLeft;
	import uk.co.teethgrinder.elements.labels.YLegendRight;
	import uk.co.teethgrinder.elements.menu.Menu;
	...
	public class Chart extends Sprite {
		...
		//private var _loadingString:String = "uk.co.teethgrinder.Loading...";
		...
		private function addedToStage(event:Event):void{
			...
			this.set_the_stage();
			if(this._chartData){
				this.build_chart(this._chartData);
			}
		}
		...
		private function mouse_move_proximity( event:MouseEvent ):void {
			...
			//var elements:Array = this.obs.mouse_move_proximity( event.localX, event.localY );
			this.tooltip.closest( elements );
		}
		
		private function mouse_move_closest( event:MouseEvent ):void {
			//trace("mouse_move_closest,mousex="+this.mouseX+",mousey="+this.mouseY);
			//var elements:Array = this.obs.closest_2( event.localX, event.localY );
			var elements:Array = this.obs.closest_2( this.mouseX, this.mouseY );
			//trace("closest elements[0].tooltip="+elements[0].get_tooltip());
			this.tooltip.closest( elements );
		}
		...
		private function resize_pie(): ScreenCoordsBase {
			...
			this.obs.resize( sc,this.width,this.height );
		...
		private function resize_chart(): ScreenCoordsBase {
			...
			this.obs.resize( sc,this._width,this._height );
		...
		private function build_right_click_menu(): void {
			...
			var smcm:ContextMenuItem = new ContextMenuItem("http://www.smithfox.com" );
			smcm.addEventListener(
				ContextMenuEvent.MENU_ITEM_SELECT,
				function doSomething(e:ContextMenuEvent):void {
					
					var url:String = "http://www.smithfox.com/";
					var request:URLRequest = new URLRequest(url);
					flash.net.navigateToURL(request, '_blank');
				});
			cm.customItems.push( smcm );
